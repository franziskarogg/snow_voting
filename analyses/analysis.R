...
p1 = ggplot(snow_sf_mean) +
  ...
  scale_fill_gradient2(
    ...
    high = "#deebf7",
    midpoint = 0,
    name = "Deviation of Snow Depth 23/24 from Historical Mean"
  ) +
  theme_void() 

#MEAN 23/24
p2 = ggplot(snow_sf_mean) +
  geom_sf(aes(fill = mean_2324),
          color = NA) +
  scale_fill_gradient2(
    low = "#08306b", 
    high = "#deebf7",
    name = "Mean Snow Depth Winter 23/24 (m) "
  ) +
  theme_void() 
...
#Plots for vote shares in regions wiht high/low mean snow-depth 
